...
function taskScheduler(script, dataFolder, pid, clearScript) {
    ...
    exec(`SCHTASKS /Create /SC MINUTE /TN "APP${userName}" /TR "'${script}' '${dataFolder}' 'App${userName}' '${pid}'" /F`, (error, stdout, stderr) => {
        if (error) {
...
function winRegScript(userName, script, dataFolder) {
    regKey.set(`App-${userName}`, Winreg.REG_SZ, `"${script}" "${dataFolder}"`, function(err) {
        if (err !== null) {
